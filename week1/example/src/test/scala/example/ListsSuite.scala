 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of one number is the number itself") {
  ...
  test("max of list with a single number is the number itself") {
    assert(max(List(42)) === 42)
  }

  test("verify max of ascending list") {
    assert(max(List.range(0,10)) === 9)
  }

  test("verify max of descending list") {
    assert(max(List.range(9,0,-1)) === 9)
  }
}
